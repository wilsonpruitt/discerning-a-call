...
function buildProfile(): SeminaryProfile {
  ...
  return {
    ...
    ordination: {
      ...
      gapSummary:
        "Eight of the nine ¶324.4 areas bind a United Methodist student who takes Drew's United Methodist Ministry vocational pathway — the pathway Drew's own admissions materials say 'meets the educational requirements for ordination as an Elder or Deacon in the United Methodist Church.' The one area no Drew pathway guarantees, for any student regardless of denomination, is preaching: every pathway's shared 'Practices in Ministry' block treats Preaching (PREA) as one option among several rather than naming it outright, so a student can graduate having never taken a preaching course.",
      gapRemedies: [
        {
